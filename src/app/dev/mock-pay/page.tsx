...
function MockPay() {
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-md flex-col justify-center px-6 py-12">
      <p className="mb-3 text-xs font-medium uppercase tracking-[0.18em] text-ink/50">Test bank · mock rails</p>
      <Card className="ledger-spine ruled-paper pl-4">
        <CardContent className="space-y-5">
          ...
          {order ? (
            ...
            <p className="text-sm text-ink/60">Loading the order…</p>
          ) : null}
...
export default function MockPayPage() {
  return (
    <Suspense fallback={<main className="mx-auto max-w-md px-6 py-12 text-sm text-ink/60">Loading…</main>}>
      <MockPay />
    </Suspense>
  );
}
