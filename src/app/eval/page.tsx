    ...
    <>
      ...
      <main className="mx-auto max-w-6xl px-6 pb-20 pt-10">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs font-medium uppercase tracking-[0.18em] text-ink/50">Scorecard · measured, not claimed</p>
        ...
        <div id="eval-status" aria-live="polite" className="mt-3 min-h-[1.25rem] text-sm">
          ...
          {!running && load.kind === "loading" ? <p className="text-ink/60">Loading the scorecard…</p> : null}
  ...
  return (
    <CardHeader>
      <div>
        ...
        <p className="mt-1 text-sm text-ink/60">Scripted adversaries try to move money against policy. A breach is money that moved anyway.</p>
      </div>
    </CardHeader>
  );
}
      ...
      <CardContent>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-ink/10 text-xs font-medium uppercase tracking-[0.12em] text-ink/50">
        ...
        <CardContent className="space-y-5">
          ...
          <div className="overflow-x-auto">
            {rt.by_category.length === 0 ? (
              <p className="py-3 text-sm text-ink/60">No attack categories recorded in this run.</p>
            ) : (
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-ink/10 text-xs font-medium uppercase tracking-[0.12em] text-ink/50">
                        ...
                        <td className="py-2.5 pl-3 text-left">
                          {top.length === 0 ? (
                            ...
                            <span className="flex flex-wrap gap-x-3 gap-y-0.5 font-mono text-xs text-ink/70">
                              {top.map((r) => (
                                <span key={r.code}>
                                  {r.code} <span className="text-ink/45">×{r.n}</span>
                                </span>
                              ))}
                            </span>
                          )}
